...
Public Class addvideo
    ...
    Dim dsaddvideo As New Doctors_VideosTableAdapter
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Dim NavigationURL = txtnavagationpathurl.Text
        If FUDocument.HasFile Then
            If FUDocument.PostedFile.ContentType <> "image/jpeg" And FUDocument.PostedFile.ContentType <> "image/pjpeg" And FUDocument.PostedFile.ContentType <> "image/gif" And FUDocument.PostedFile.ContentType <> "application/pdf" And FUDocument.PostedFile.ContentType <> "application/msword" And FUDocument.PostedFile.ContentType <> "application/vnd.openxmlformats-officedocument.wordprocessingml.document" Then
                LblMsg.Text = "Files of type[" & FUDocument.PostedFile.ContentType & "] are not allowed, Only Images are allowed"
            ElseIf FUDocument.PostedFile.ContentLength > "5242880" Then
                LblMsg.Text = "Image size should not exceed 5MB"
            Else
                FUDocument.PostedFile.SaveAs(Server.MapPath("~/BannerDocuments") & "/" & FUDocument.PostedFile.FileName)
                NavigationURL = System.Configuration.ConfigurationManager.AppSettings("WebsiteURL") & "BannerDocuments/" & FUDocument.PostedFile.FileName
            End If
        End If
    ...
    Protected Sub GVAddVideo_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GVAddVideo.RowCommand
        If e.CommandName = "CmdEdit" Then
            ...
            If Dtaddvideo.Rows.Count > 0 Then
                ...
                txtnavagationpathurl.Text = dtrow("iframeurl")
                txttargeturl.Text = dtrow("target")
    ...
    Private Sub BuildQuery()
        ...
        If (txtnavagationpathurl.Text <> "") Then
            If strsql = "" Then
                strsql = " where Lower(iframeurl) like '%" & txtnavagationpathurl.Text.ToLower() & "%'"
            Else
                strsql = strsql & " or Lower(iframeurl) like '%" + txtnavagationpathurl.Text.ToLower() & "%'"
            End If
        End If

        If (txttargeturl.Text <> "") Then
            If strsql = "" Then
                strsql = " where Lower(target) like '%" & txttargeturl.Text.ToLower() & "%'"
            Else
                strsql = strsql & " or Lower(target) like '%" + txttargeturl.Text.ToLower() & "%'"
            End If
        End If
