...
Imports System.IO
Public Class managenewschannels
    ...
    Protected Sub GridView1_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridView1.DataBound
        For i = 0 To GridView1.Rows.Count - 1
            ...
            If Not (LblLogo) Is Nothing Then
                ImageUrl = "javascript:window.open('/MediaFiles/newschannellogo/" & LblLogo.Text & "','NewsImage','status=0,toolbar=0,menubar=0,location=0,scrollbars=0,resizable=0,width=400,height=200'); window.close();"
    ...
    Protected Sub GridView1_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridView1.RowCommand
        If e.CommandName = "CmdActive" Then
            If GridView1.Rows(e.CommandArgument).Cells(3).Text = "Active" Then
                Dsnewschannels.UpdateActiveStatus("N", GridView1.DataKeys(e.CommandArgument).Value)
            Else
                Dsnewschannels.UpdateActiveStatus("Y", GridView1.DataKeys(e.CommandArgument).Value)
            End If
            GridView1.DataBind()
    ...
    Protected Sub GridView1_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles GridView1.RowUpdating
        If CInt(Dsnewschannels.CheckOtherPrimary(e.NewValues(0), e.Keys(0))) > 0 Then
            LblMsg.Text = "News Paper [ " & e.NewValues(0) & " ] Already Exisits, Try With The Other !"
            e.Cancel = True
        Else
            Dsnewschannels.UpdateNewsChannelName(e.NewValues(0), e.Keys(0))
            ...
            If FULogoo.HasFile Then
                ...
                Else
                    ...
                    FileName = Dsnewschannels.GetNewsChannelsLogoById(e.Keys(0))

                    Try
                        Dim TheFile As New FileInfo(Server.MapPath("~/MediaFiles/newschannellogo") & "/" & FileName)
                        If TheFile.Exists Then
                            File.Delete(Server.MapPath("~/MediaFiles/newschannellogo") & "/" & FileName)
                        Else
                    ...
                    FULogoo.PostedFile.SaveAs(Server.MapPath("~/MediaFiles/newschannellogo") & "/" & LogoName)

                    Dsnewschannels.UpdateNewsChannelLogo(LogoName, e.Keys(0))
    ...
    Protected Sub BtnSave_Click(sender As Object, e As EventArgs) Handles BtnSave.Click
        If CInt(Dsnewschannels.CheckPrimary(TxtchannelName.Text)) > 0 Then
            LblMsg.Text = "Channel [" & TxtchannelName.Text & "] Already Exists, Please Try With The Other !"
        Else
            If FULogo.HasFile Then
                ...
                Else
                    LogoName = Replace(TxtchannelName.Text, " ", "")
                    LogoName = Replace(LogoName, "&", "") & "." & Right(FULogo.PostedFile.FileName, 3)

                    FULogo.PostedFile.SaveAs(Server.MapPath("~/MediaFiles/newschannellogo") & "/" & LogoName)

                    Dsnewschannels.Insert(TxtchannelName.Text, LogoName, "Y")
